...
const Description = () => {
  ...
  useEffect(() => {
    const getPokemonDetail = async () => {
      const res = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${pokemonSelected}`
      );
  ...
  if (!pokemon) {
    return <div> Esperando Respuesta de la aplicación ...</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="button-container">
        <Button variant="primary" onClick={goBackToSelection}>Intenta seleccionando otro Pokemón</Button>
      </div>
    </div>
  );
};
